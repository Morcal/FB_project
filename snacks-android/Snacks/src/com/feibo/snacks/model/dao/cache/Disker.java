...
class Disker {
    ...
    private void initDir(Context context, String path) {
        ...
        cacheDir = DirContext.getInstance().getDir(DirEnum.CACHE);
        imageDir = DirContext.getInstance().getDir(DirEnum.IMAGE);
    }

    public synchronized void putStringToDisk(String content, String name) {
    ...
    public synchronized void deleteDiskString(String name) {
        File file = getFile(cacheDir, name);
        if (file.exists()) {
            file.delete();
        }
